...
$param_kode_barang=$_GET['id'];
$sesi_tanggal=$_SESSION['tgl'];
$query_awal=mysql_query("Select min(tgl) as awal from barang_keluar where kode_barang='".$param_kode_barang."'");
...
$query_cari_count=mysql_query("Select count(id_keluar) as numrows from barang_keluar Where kode_barang='".$param_kode_barang."' and tgl between '".$tglawal."' and '".$tglakhir."'");
...
<?php
	...
	$query_cari=mysql_query("Select sum(jumlah) as jml,jumlah from barang_keluar Where kode_barang='".$param_kode_barang."' and tgl between '".$tglawal."' and '".$tglakhir."'");
	...
	$query_barang=mysql_query("Select * from barang_keluar Where kode_barang='".$param_kode_barang."' and tgl between '".$tglawal."' and '".$tglakhir."'");	
	$sd=array();
	while($rww=mysql_fetch_array($query_barang))
	{
		$sd[]=$rww['jumlah'];
	}
	$query_persediaan=mysql_query("Select * from data_persediaan where kode_barang='".$param_kode_barang."'");
...
echo '<p></p>';
	...
	if(isset($_GET['f']))
	{
		...
		if(isset($_POST['simpan']))
		{
			...
			$q=mysql_query("Insert into data_perencanaan (`tgl`, `kode_barang`, `rata`, `lead`, `safety`, `tersedia`, `order`, `foi`) values ('".$tglakhir."','".$_GET['id']."','".$_POST['rata_pemakaian2']."','".$_POST['lead2']."','".$_POST['safety2']."','".$_POST['tersedia2']."','".$_POST['order_interval2']."','".$_POST['foi2']."')") or die(mysql_error());
